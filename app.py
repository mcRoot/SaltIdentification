...
def encode_layer_resnet(input=None, feature_maps=32, initializer=None, activation=tf.nn.relu, training=None, max_pooling=True):
    p = tf.layers.conv2d(input, feature_maps, config.kernel_size, kernel_initializer=initializer, padding="same", activation=None)
    p = tf.layers.batch_normalization(p, training=training, momentum=config.momentum)
    p = activation(p)
    #p = tf.layers.conv2d(p, feature_maps // 4, config.kernel_size, kernel_initializer=initializer, padding="same", activation=None)
    #p = tf.layers.batch_normalization(p, training=training, momentum=config.momentum)
    #p = activation(p)
    p = tf.layers.conv2d(p, feature_maps, config.kernel_size, kernel_initializer=initializer, padding="same", activation=None)
    p = tf.layers.batch_normalization(p, training=training, momentum=config.momentum)
...
def build_net():
    ...
    p, _ = encode_layer(input=p, feature_maps=64, initializer=initializer, training=training, max_pooling=False)
    p, _ = encode_layer(input=p, feature_maps=128, initializer=initializer, training=training)
    c1, _ = encode_layer(input=p, feature_maps=128, initializer=initializer, training=training, max_pooling=False)
    ...
    p, _ = encode_layer(input=p, feature_maps=256, initializer=initializer, training=training)
    c2, _ = encode_layer(input=p, feature_maps=256, initializer=initializer, training=training,  max_pooling=False)
    ...
    c3, _ = encode_layer(input=p, feature_maps=512, initializer=initializer, training=training,  max_pooling=False)
